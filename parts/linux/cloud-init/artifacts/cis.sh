...
replaceOrAppendSetting() {
    ...
    if grep --extended-regexp "$SEARCH_PATTERN" "$FILE">/dev/null ; then
        sed --regexp-extended --in-place "s|${SEARCH_PATTERN}|${SETTING_LINE}|g" "$FILE" || exit $ERR_CIS_APPLY_PASSWORD_CONFIG
    else
        echo -e "\n${SETTING_LINE}" >> "$FILE"
    fi
    ...
    if [[ $(grep --extended-regexp "$SEARCH_PATTERN" "$FILE") != "$SETTING_LINE" ]]; then
...
#   '.*$'           -- Then 0 or nore of any character which is the end of the line. Note that this allows for a setting value to be there or not.
#
# This is based on a combination of the syntax for the file and real examples we've found.
